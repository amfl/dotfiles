...
install-pkg() {
  ...
  SOFTWARE="vim stow git tig tmux cmake zsh inotify-tools ncdu htop tree dictd jq"

  case $OS in
    "Linux")
      PKGMAN="apt-get install"
      SOFTWARE="$SOFTWARE build-essential silversearcher-ag ack-grep dict-gcide"
      ;;
    "Darwin")
      ;;
    "FreeBSD")
      PKGMAN="pkg install"
      SOFTWARE="$SOFTWARE the_silver_searcher ack dictd-database-20031009"
      ;;
    *)
      echo "Unknown OS: $OS"
      exit
